...
import 'package:core/src/data/sources/log_data_source.dart';

enum LogLevel { debug, error, info }

class LoggerService {
  final LogDataSource _logDataSource = GetIt.I<LogDataSource>();
  ...
  Future<void> info({required String featureArea, required String message, Map<String, dynamic>? metadata}) async {
  ...
  Future<void> _persistToDatabase({
    ...
    try {
      ...
      await _logDataSource.logError(
        ...
        featureArea: featureArea,
        metadata: {...?metadata, 'log_level': level.name.toUpperCase()},
      );
    } catch (e) {
