...
CREATE OR REPLACE FUNCTION public.fn_register_invitation(p_community_id uuid, p_user_id uuid, p_property_id uuid, p_visitor_id uuid, p_from_date timestamp with time zone, p_to_date timestamp with time zone)
 RETURNS void
 LANGUAGE plpgsql
AS $function$
BEGIN
...
CREATE OR REPLACE FUNCTION public.fn_register_visitor(p_community_id uuid, p_user_id uuid, p_name text, p_identification_path text)
 RETURNS void
 LANGUAGE plpgsql
AS $function$
BEGIN
